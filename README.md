...
Thiccdal is a **streaming command and control system** built with .NET 10 and Blazor Server. It runs on the stream PC and is operated remotely from a second device — typically a Surface Pro tablet — giving the streamer hands-free control over chat, the chatbot, and the live overlay.
...
- **Pre-live checklist** — readiness checks and a go-live action that opens a tracked stream session

Video ingest, restreaming, and recording are out of scope: OBS talks to each platform directly.
