...
public class TwitchTokenManagerTests
{
    ...
    [Fact]
    public async Task WhenNoTokenExists_GetTokenThrows()
    {
        var manager = CreateManager(nameof(WhenNoTokenExists_GetTokenThrows));

        await Assert.ThrowsAsync<InvalidOperationException>(() => manager.GetToken());
    }
