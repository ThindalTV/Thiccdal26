...
/// <summary>
/// Coordinates ingest lifecycle with local disk recording in the standalone RTMP server process.
/// </summary>
public sealed class StreamingService : IStreamingService
{
    ...
    public async Task Start(CancellationToken cancellationToken = default, Guid? sessionId = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(_holder.GetCurrent().IngestUrl))
        {
            throw new InvalidOperationException("IngestUrl must be configured before going live.");
        }
